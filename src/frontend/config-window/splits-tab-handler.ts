...
function appendAllGoldPaces(goldPaceSelection: HTMLElement, goldPaceTimes: { splitIndex: number, time: number; }[], levelMappings: { splits: string[] }) {
    ...
    levelMappings.splits.forEach((split, index) => {
        ...
        addEnterAndWaitValidator(input, async (value) => await validateGoldenPace(value, index), "SPLIT_INVALID_FORMAT_OR_TOO_SLOW", split);
...
function appendSplit(name: string, from: number, to: number, nodes: Node[], goldSplitTimes: {
    ...
    addEnterAndWaitValidator(input, async (value) => await validateGoldSplit(input.value, from, to), "SPLIT_INVALID_FORMAT_OR_TOO_SLOW", name);
