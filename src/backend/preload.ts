...
contextBridge.exposeInMainWorld('electronAPI', {
    ...
    redrawOverlay: (callback: (event: IpcRendererEvent, pbRunInfoAndSoB: PbRunInfoAndSoB, reverseSplits: boolean) => void) => ipcRenderer.on('redraw-split-display', callback),
    onSplitPassed: (callback: (event: Electron.IpcRendererEvent, splitInfo: SplitPassedInfo) => void) => ipcRenderer.on('split-passed', callback),
    ...
    onStatusChanged: (callback: (event: IpcRendererEvent, statusMsg: OverlayStatus) => void) => ipcRenderer.on('status-changed', callback),
